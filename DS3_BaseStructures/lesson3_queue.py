...
    先进先出(FIFO)：最新被加入的原色处于队尾，在队列中停留的最长时间的元素处于队首
    ...
    抽象数据类型（ADT）:
        ...
        enqueue(item) 将数据添加到队尾，无返回值
        dequeue () 从对首移出数据项，无需参数，返回值为对首数据项
...
'''
    ...
    过程：
        ...
        2 一个小时中的每一秒（currentSecond）都需要判断：
          是否有新的打印任务生成如果有，把它加入打印队列：
          如果打印机空闲并且队列不为空：
          1 从队列中拿出一个任务交给打印机
          2 从加入打印机时间-加入队列的时间=等待时间
          3 将该任务的等待时间加入到一个列表中，方便后续使用，计算总的学生打印花费的时间
